...
import { ArrowRight, Calendar, Check, MapPin, MoreVertical, Share2, Trash2 } from "lucide-react";
...
function destinationGradient(destination: string): (typeof COVER_GRADIENTS)[number] {
  let hash = 0;
...
export function TripCard({ trip, coverPhoto }: TripCardProps) {
  ...
  const itemCount =
    trip.itineraries?.[0]?.days.reduce((acc, day) => acc + day.items.length, 0) ?? 0;
  const dayCount = trip.itineraries?.[0]?.days.length ?? 0;

  const start = trip.startDate ? new Date(trip.startDate) : null;
  const end = trip.endDate ? new Date(trip.endDate) : null;
  const tripDuration =
    start && end ? differenceInDays(end, start) + 1 : null;
